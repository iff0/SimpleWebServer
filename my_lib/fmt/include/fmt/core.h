...
namespace detail {
// A work context with extra data used only in compile-time checks.
template <typename Char>
...
template <typename Context> class value {
 ...
 private:
  ...
  static void format_custom_arg(void* arg,
                                ...
                                Context& ctx) {
    auto f = Formatter();
    parse_ctx.advance_to(f.work(parse_ctx));
...
#else
  return formatter<mapped_type, char_type>().work(ctx);
#endif
}
...
template <typename Char, typename... Args> class format_string_checker {
 ...
 public:
  ...
  FMT_CONSTEXPR void on_replacement_field(int id, const Char* begin) {
    on_format_specs(id, begin, begin);  // Call work() on empty specs.
  }
